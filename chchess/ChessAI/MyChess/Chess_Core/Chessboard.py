...
class Chessboard(object):
    ...
    def export2file(self,name):
        screen = ""
        for i in range(9, -1, -1):
            row = []
            for j in range(9):
                if self.__chessmans[j][i] != None:
                    row.append(self.__chessmans[j][i].name_cn)
                else:
                    row.append("   .   ")
            line = "|".join(row) + "\n"
            screen += line
        ...
        for i in range(len(temp)):
            ...
            for j in range(len(ids)):
                if ids[j] == " 车l红 ":
                    ...
                    red_rook_left.add_to_board(j, 9-i)
                if ids[j] == " 车r红 ":
                    red_rook_right = Chessman.Rook(" 车r红 ", "red_rook_right", True, self)
                    red_rook_right.add_to_board(j, 9-i)
                if ids[j] == " 车l黑 ":
                    black_rook_left = Chessman.Rook(" 车l黑 ", "black_rook_left", False, self)
                    black_rook_left.add_to_board(j, 9-i)
                if ids[j] == " 车r黑 ":
                    black_rook_right = Chessman.Rook(" 车r黑 ", "black_rook_right", False, self)
                    black_rook_right.add_to_board(j, 9-i)
                if ids[j] == " 马l红 ":
                    red_knight_left = Chessman.Knight(" 马l红 ", "red_knight_left", True, self)
                    red_knight_left.add_to_board(j, 9-i)
                if ids[j] == " 马r红 ":
                    red_knight_right = Chessman.Knight(" 马r红 ", "red_knight_right", True, self)
                    red_knight_right.add_to_board(j, 9-i)
                if ids[j] == " 马l黑 ":
                    black_knight_left = Chessman.Knight(" 马l黑 ", "black_knight_left", False, self)
                    black_knight_left.add_to_board(j, 9-i)
                if ids[j] == " 马r黑 ":
                    black_knight_right = Chessman.Knight(" 马r黑 ", "black_knight_right", False, self)
                    black_knight_right.add_to_board(j, 9-i)
                if ids[j] == " 炮l红 ":
                    red_cannon_left = Chessman.Cannon(" 炮l红 ", "red_cannon_left", True, self)
                    red_cannon_left.add_to_board(j, 9-i)
                if ids[j] == " 炮r红 ":
                    red_cannon_right = Chessman.Cannon(" 炮r红 ", "red_cannon_right", True, self)
                    red_cannon_right.add_to_board(j, 9-i)
                if ids[j] == " 炮l黑 ":
                    black_cannon_left = Chessman.Cannon(" 炮l黑 ", "black_cannon_left", False, self)
                    black_cannon_left.add_to_board(j, 9-i)
                if ids[j] == " 炮r黑 ":
                    black_cannon_right = Chessman.Cannon(" 炮r黑 ", "black_cannon_right", False, self)
                    black_cannon_right.add_to_board(j, 9-i)
                if ids[j] == " 相l红 ":
                    red_elephant_left = Chessman.Elephant(" 相l红 ", "red_elephant_left", True, self)
                    red_elephant_left.add_to_board(j, 9-i)
                if ids[j] == " 相r红 ":
                    red_elephant_right = Chessman.Elephant(" 相r红 ", "red_elephant_right", True, self)
                    red_elephant_right.add_to_board(j, 9-i)
                if ids[j] == " 象l黑 ":
                    black_elephant_left = Chessman.Elephant(" 象l黑 ", "black_elephant_left", False, self)
                    black_elephant_left.add_to_board(j, 9-i)
                if ids[j] == " 象r黑 ":
                    black_elephant_right = Chessman.Elephant(" 象r黑 ", "black_elephant_right", False, self)
                    black_elephant_right.add_to_board(j, 9-i)
                if ids[j] == " 仕l红 ":
                    red_mandarin_left = Chessman.Mandarin(" 仕l红 ", "red_mandarin_left", True, self)
                    red_mandarin_left.add_to_board(j, 9-i)
                if ids[j] == " 仕r红 ":
                    red_mandarin_right = Chessman.Mandarin(" 仕r红 ", "red_mandarin_right", True, self)
                    red_mandarin_right.add_to_board(j, 9-i)
                if ids[j] == " 仕l黑 ":
                    black_mandarin_left = Chessman.Mandarin(" 仕l黑 ", "black_mandarin_left", False, self)
                    black_mandarin_left.add_to_board(j, 9-i)
                if ids[j] == " 仕r黑 ":
                    black_mandarin_right = Chessman.Mandarin(" 仕r黑 ", "black_mandarin_right", False, self)
                    black_mandarin_right.add_to_board(j, 9-i)
                if ids[j] == " 帅 红 ":
                    red_king = Chessman.King(" 帅 红 ", "red_king", True, self)
                    red_king.add_to_board(j, 9-i)
                if ids[j] == " 将 黑 ":
                    black_king = Chessman.King(" 将 黑 ", "black_king", False, self)
                    black_king.add_to_board(j, 9-i)
                
                for k in range(5):
                    if ids[j] == " 兵{}红 ".format(k+1):
                        red_pawn_1 = Chessman.Pawn(" 兵{}红 ".format(k+1), "red_pawn_{}".format(k+1), True, self)
                        red_pawn_1.add_to_board(j, 9-i)

                for k in range(5):
                    if ids[j] == " 卒{}黑 ".format(k+1):
                        black_pawn_1 = Chessman.Pawn(" 卒{}黑 ".format(k+1), "black_pawn_{}".format(k+1), False, self)
                        black_pawn_1.add_to_board(j, 9-i)
